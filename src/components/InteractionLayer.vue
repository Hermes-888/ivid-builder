				...
				switch(cues[i].id) {
					...
					case 'ImageButton':
						...
						mcInstance.$on('imageSelected', function(mcData) {
							console.log('imageSelected:', mcData);
							// if points, update the scoreboard
							if (mcData.points) {
								comp.$emit('updateScore', mcData.points);
							}
							comp.removeComponents(mcData);
						});
						break;
					...
					case 'Event':
						/**
						 * trigger an event on a child component
						 * listen for specific events on the child mcInstance
						 */
						console.log('Event:', data);
						comp.$emit(data.message, data.data);
						break;
					case 'Scoreboard':
						...
						// Scoreboard has a listener to increment the score
						comp.$on('updateScore', function(data) {
							console.log('Scoreboard mcInstance emit:', data);
							mcInstance.$emit('updateScore', data);
						});

						this.addComponents(mcInstance);
						mcInstance.$on('removeScoreboard', function(mcData) {
							// console.log('Scoreboard Removed:', mcData);
							comp.removeComponents(mcData);
						});
						break;
					case 'TitleScreen':
